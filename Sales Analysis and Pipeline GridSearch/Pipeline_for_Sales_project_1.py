...
print("________________using GRIDSEARCH ____________________")
import numpy as np
import pandas as pd
from sklearn.base import BaseEstimator, TransformerMixin
from sklearn.compose import ColumnTransformer
...
# --- 1. Custom Sklearn-Compatible IQR Capper ---
class IQRCapper(BaseEstimator, TransformerMixin):
    def __init__(self, factor=1.5):
        self.factor = factor
...
# --- 2. Data Setup & Split ---
num_cols_part_2 = [
    'tenure_years', 'total_late_days', 'total_leaves_taken',
    'avg_performance_score', 'job_level', 'total_projects', 'salary_change_count'
]
num_cols_to_cap = ['total_salary_increase', 'total_absences']
cate_col = ['department_name', 'gender']
...
preprocessor = ColumnTransformer(
    transformers=[
        ('skewed_num', combined_pipeline, num_cols_to_cap),
        ('plain_num', StandardScaler(), num_cols_part_2),
        ('cat', OneHotEncoder(sparse_output=False, handle_unknown='ignore'), cate_col)
    ]
)

# Place 'Ridge()' as a default placeholder model
full_pipeline = Pipeline([
    ('preprocessor', preprocessor),
    ('model', Ridge())
])

# --- 4. Define Multi-Model Parameter Grid ---
# Each dictionary in the list defines a separate search space for a specific model family.
param_grid = [
    # Search space 1: Ridge Regression
    {
        'preprocessor__skewed_num__capper__factor': [1.5, 2.0, 3.0],
        'model': [Ridge(random_state=42)],
        'model__alpha': [0.01, 0.1, 1.0, 10.0, 100.0]
    },
    # Search space 2: Lasso Regression
    {
        'preprocessor__skewed_num__capper__factor': [1.5, 2.0, 3.0],
        'model': [Lasso(random_state=42, max_iter=5000)],
        'model__alpha': [0.001, 0.01, 0.1, 1.0, 10.0]
    },
    # Search space 3: ElasticNet Regression
    {
        'preprocessor__skewed_num__capper__factor': [1.5, 2.0, 3.0],
...
# --- 6. Results Summary & Test Evaluation ---
...
print(f"Test Log-Scale R2 Score: {r2:.4f}")
print(f"Test Actual Dollars MAE: ${mae:,.2f}")
